...
public class WrongBTree<Key extends Comparable<Key>, Value>
{
    ...
    private static final int MAX = 4; //CHANGING THIS TO 2
    ...
    private Entry get(Node currentNode, Key key, int height)
    {
        ...
        else
        {
            ...
            //didn't find the key
            return null;//  NOT SURE WHY I NEED THIS WHEN I'M RETURNING THE SMALL GET
        }
    }
        ...
        else
        {
            ...
            for (j = 0; j < currentNode.entryCount; j++)
            {
                ...
                if ((j + 1 == currentNode.entryCount) || less(key, currentNode.entries[j + 1].key))
                {
                    ...
                    Node newNode = this.put(currentNode.entries[j++].child, key, val, height - 1); //let's see if I delete++
                    if (newNode == null)
                    {
                        return null;
                    }
...
public class DocumentPersistenceManager implements PersistenceManager<URI, Document> {
    ...
    @Override
    public void serialize(URI uri, Document val) throws IOException { //still need to take care of byteArray Serialize
        Gson gson = new GsonBuilder().registerTypeAdapter(Document.class, new mySerial()).create();
        ...
        String path = uri.toString().substring(7, uri.toString().length());
    ...
    @Override
    public boolean delete(URI uri) throws IOException { //what is the is method supposed to do in the context of documentStore??
    ...
    private class myDeserial implements JsonDeserializer<Document>{
        ...
        public Document deserialize(JsonElement arg0, Type arg1, JsonDeserializationContext arg2) throws JsonParseException {
            JsonObject jsonObject = arg0.getAsJsonObject();//well, need to look at my phone when we get back
            Type type = new TypeToken<HashMap<String,Integer>>(){}.getType();
            try {
                Gson gson = new Gson();
                byte[] bytes = gson.fromJson(jsonObject.get("bytes"), byte[].class);
                Document doc = jsonObject.get("txt") != null?
                new DocumentImpl(new URI(jsonObject.get("uri").getAsString()), jsonObject.get("txt").getAsString()):
                new DocumentImpl(new URI(jsonObject.get("uri").getAsString()), bytes);
                Map<String, Integer> map = gson.fromJson(jsonObject.get("map"), type);
                doc.setWordMap(map);
                return doc; 
